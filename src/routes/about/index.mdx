...
- Outcome-oriented > Project oriented
  ...
  - If you're not outcome-oriented then you're pleasing stakeholders, not building products
- Developer velocity == Business Value Created / Time
  - If finding business value is a search algorithm then research is the best heuristic
  - Spend less time on code by decreasing time to feedback; faster test suites, compile time, UI updates, deployments, customer feedback
  - Spend less time on overhead
    - DevOps is overhead
    - Regression testing is overhead
    - Writing code is overhead
...
- You only know for sure who someone truly is when they are exhausted, thwarted, the stakes are high and nobody is looking.
  ...
  - This is hard to mimic in the private sector. Startups are a decent approximation.